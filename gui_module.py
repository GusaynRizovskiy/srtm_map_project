...
class RadioApp(ctk.CTk):
    ...
    def _setup_ui(self):
        ...
        self.sidebar = ctk.CTkScrollableFrame(self, width=340, label_text="Параметры системы")
        ...
        # --- 4. ОБОРУДОВАНИЕ (Скорректировано) ---
        ...
        ctk.CTkLabel(self.ant_frame, text="Конструкция антенны:").pack(pady=(5, 0))
    ...
    def create_group(self, title):
        ...
        ctk.CTkLabel(frame, text=title, font=("Arial", 13, "bold"), text_color="#72a1d2").pack(pady=5)
        return frame

    def create_field(self, master, label_text, default_val):
        ctk.CTkLabel(master, text=label_text, justify="left", anchor="w").pack(pady=(5, 0), padx=10, fill="x")
    ...
    def show_profile_window(self):
        ...
        try:
            ...
            # Используем частоту именно для Френеля
            f_frenel = float(self.f_frenel_entry.get())
            # Макс. частота (для справки или будущих расчетов потерь)
        ...
        fig_p = Figure(figsize=(8, 5), dpi=100)
        ax_p = fig_p.add_subplot(111)

        ax_p.fill_between(dist, earth_arc, -100, color='#d2b48c', alpha=0.2, label='Кривизна Земли')
        ax_p.fill_between(dist, elev_curved, earth_arc, color='sienna', alpha=0.7, label='Рельеф')
        ax_p.fill_between(dist, los_line - f_radius, los_line + f_radius, color='yellow', alpha=0.2,
                          label=f'Зона Френеля ({f_frenel} ГГц)')
        ax_p.plot(dist, los_line, 'b--', label='Линия LOS', lw=1.5)

        ax_p.set_title(f"Профиль трассы. Раб. частота: {f_max} МГц")
