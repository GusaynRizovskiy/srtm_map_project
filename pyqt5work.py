...
class ProfilePlotter(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        button_show_profile = QPushButton("Отобразить профиль")
        button_show_profile.clicked.connect(self.show_profile)

        button_clear_values = QPushButton("Очистить значения")
        button_clear_values.clicked.connect(self.clear_values)

        layout = QVBoxLayout()
        layout.addWidget(self.canvas)
        layout.addWidget(button_show_profile)
        layout.addWidget(button_clear_values)
    ...
    def plot_elevation_map(self):
        ...
        # Добавление цветовой шкалы к текущей фигуре и оси
        self.canvas.figure.colorbar(cax, ax=self.ax, label='Elevation (meters)')
    ...
    def show_profile(self):
        if len(self.selected_points) == 2:
            ...
            self.ax.clear()  # Очищаем оси перед новым графиком

            distance_kilometers = distance_meters / 1000.0
            self.ax.plot(np.linspace(0, distance_kilometers, num_points), elevations)
            ...
            self.ax.plot(np.linspace(0, distance_kilometers, num_points), ellipse_y_upper,
